    ...
    public static class ReadConsole
    {
        ...
        public static object HandleCommand(JObject @params)
        {
            ...
            try
            {
                ...
                action = p.Get("action", "get").ToLower();
            ...
            {
                string actionLabel = action ?? "unknown";
                McpLog.Error($"[ReadConsole] Action '{actionLabel}' failed: {e?.Message ?? e?.ToString() ?? "Unknown error"}");
                return new ErrorResponse($"Internal error processing action '{actionLabel}': {e?.Message ?? "Unknown error"}");
            }
        }
